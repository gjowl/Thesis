...
if __name__ == '__main__':
    ...
    for seq in wt['Sequence'].unique():
        ...
        # get the mutant with the biggest difference in SASA
        tmp_mut = tmp_mut[tmp_mut['Mutant_DimerSasa'] - tmp_mut['WT_DimerSasa'] == (tmp_mut['Mutant_DimerSasa'] - tmp_mut['WT_DimerSasa']).max()]
    ...
    for i in range(len(void_df['WT_MUT'].unique())):
        ...
        # add the number of points to the plot
        #plt.text(i+1, 0.5, f'{len(y)}', ha='center', va='bottom')
    # remove the title
